...
- `GET /api/today/videos?day=` — per-video summary (status, ReID, frames indicator, processing time, speed_kmh, pipeline_error)
...
- `GET /api/today/video/{basename}/pose` — pose clip URLs (when POSE_ENABLED)
- `GET /api/today/video/{basename}/full` — full source video URL (only if the source file currently exists on disk)
- `GET /api/today/gate-crossings?day=` — videos with ReID crops: basename, time, direction, persons_up/down, away_back, scores, crop URLs
- `GET /api/today/stats?day=` — aggregated stats for a day. For today, open `away_intervals` (started but not yet closed) get `dur` filled with elapsed time (`now − start`) and an `ongoing: true` flag.
- `GET /api/stats/overall` — overall stats with heatmaps. Time-of-day and weekday heatmaps use the union of (log-file days, cached days) excluding today so the pattern window survives log retention. Per-day video counts remain log-files only. Weekday-heatmap cells also include per-bin `away_occurrences` / `back_occurrences` (lists of `{date, hhmmss}` for each weekday × hour bin).
- `GET /api/stats/reid` — ReID auto-detection accuracy: per-day TP / FP / FN, derived precision / recall / F1, average ReID match score (TP + FN + FPFN), and the seven-day moving average for all of the above. Each per-day entry also carries an `events` list `[{video, hhmmss, kind, score, crop_url}]` with `kind ∈ {TP, FP, FN, FPFN}` for the recognition-wall view in the Android app. Per-day rows are persisted in `temp/reid_metrics_cache.json` (version 3) and never pruned, so accuracy history survives log retention.
...
- `GET /api/events/latest?since=` — away/back events with current home/away status (computed from the latest event by real-world event time, not log/processing order). Also returns `next_prediction` when a same-weekday pattern is confident enough: `{kind: "away"|"back", predicted_hhmm, confidence, basis_count, basis_total, imminent}`. Skipped when the weekday has fewer than 4 historical samples, no remaining-of-day bin clears the 30% confidence threshold (even with a 2-bin rolling-window fallback), the prediction is further than 3 hours away, or — for back predictions — the candidate time is within 20 min of the current away start. `imminent: true` means the chosen bin's mean already passed but the bin is still in progress.
- `POST /api/reid/copy` — copy a ReID crop to positive or negative gallery
- `GET /api/gallery/{target}/{filename}` — serve a gallery reference crop (`target` is `positive` or `negative`)
- `DELETE /api/gallery/{target}/{filename}` — delete a gallery reference crop; the ReID embedding cache rebuilds automatically on the next run
- `GET /api/image/{basename}` — serve images (crops, frames) from TEMP_DIR
- `GET /api/highlight/{basename}` — serve highlight and pose clips from TEMP_DIR daily dirs
...
- ReID crops, insignificant frames, highlight clips, and pose clips are served from `TEMP_DIR` daily subdirectories (`YYYYMMDD/`). A media index over those folders is cached for `TEMP_MEDIA_INDEX_TTL` seconds (default 15) to avoid repeated directory scans on each request.
- `pipeline_error` flag in the per-video summary preserves the real motion status (e.g. `gate_crossing`) when a *post*-detection failure (telegram send, etc.) logs `Status: error` — the underlying status is kept and `pipeline_error: true` is surfaced separately.
- Away/back interval pairing uses log sequence index for "Reaction removed" markers, so corrected/manual reactions logged in the same minute as an auto-decline are no longer dropped.