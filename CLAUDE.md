...
Each appended frame: among persons whose bbox center is within the gate band, the **most-recent line crosser** wins the single PiP slot (`event_last_cross_frame`, recorded at the visual-crossing check), so when several people cross one-by-one the PiP hands off to whoever just crossed even if an earlier, larger person is still lingering in the band; ties / not-yet-crossed people fall back to nearest-to-line then largest box. **Hysteresis** (`prev_pip_eid`) holds the current subject while it stays in the band unless another candidate crosses *more recently* — this kills frame-to-frame flicker between two people who are near the line but haven't crossed yet, while still triggering the hand-off on an actual crossing. The chosen person is cropped from a **clean (un-annotated) frame copy** (`clean_frame_overlay`, reusing the existing `frame_for_reid` copy when present), zoomed (aspect-preserving) into the lower-right corner **above** the event text — `draw_event_overlay` returns its box top-y so the PiP anchors above it — with a two-line "funnel" connector (box top-right→PiP top-left, box bottom-right→PiP bottom-left). The PiP border and funnel lines use the **same color as the drawn bounding box**: `COLOR_PERSON` (green) normally, `COLOR_HIGHLIGHT` (red) when the person is in `LINE_Y_TOLERANCE` or within the `HIGHLIGHT_WINDOW_FRAMES` highlight window. The connector lines are drawn before the crop/background so they clip cleanly at the PiP edge. Helper: `draw_crop_overlay()`.

Visibility is positional, not crossing-confirmed: the PiP shows while the person's center is within the gate band (so it appears as they approach/cross and disappears once they move away), not while they linger far from the gate. The band defaults to `(LINE_Y_TOLERANCE + REID_LINE_EXTRA_TOLERANCE) * GATE_CROP_OVERLAY_BAND_SCALE` (resolution-aware); the bare tolerance is only a few frames wide, so the scale (default `4.0`) is the knob that keeps the PiP on screen long enough. **Visibility linger** (`GATE_CROP_OVERLAY_LINGER_FRAMES`, default `10`, `overlay_visible_until`): once the subject has been in-band, brief dips back out of the band (bbox-center jitter at the edge) keep the PiP on — using the subject's current still-tracked box — for up to N frames, so the visibility threshold doesn't blink the PiP on/off. The hand-off / selection is separate (`persons_by_eid` provides the lingering box even when out of band).

**Drawn on the downscaled output frame, cropped from the native source.** The PiP is composited *after* the per-frame resize-to-1080p (in the append block), but the crop is taken from the native (e.g. 4K) `clean_frame_overlay` and resized **once, directly** to its on-screen size. This avoids the upscale-then-frame-downscale double-resample of drawing on the 4K frame — a 4K crop shown at native size is already ~2x vs the 1080p output and stays sharp. `draw_crop_overlay` takes a `scale` (source→target) arg: it crops in source coords, then draws the PiP/funnel/border in target coords with `OVERLAY_PAD_X/PAD_Y/BOX_THICKNESS` scaled by it so margins/thickness match the downscaled boxes and text (`text_top_y` is likewise scaled at the call site). By default the PiP is never enlarged beyond the native crop's pixels (pure downscale = sharpest); `GATE_CROP_OVERLAY_ALLOW_UPSCALE=true` lifts that so `WIDTH_FRAC` can drive a larger (upscaled) PiP.

Key env vars (master, read at module load): `GATE_CROP_OVERLAY_ENABLED` (default `true`), `GATE_CROP_OVERLAY_WIDTH_FRAC` (default `0.18`, PiP width as a fraction of frame width), `GATE_CROP_OVERLAY_BAND` (absolute px override; `-1` = derive at runtime), `GATE_CROP_OVERLAY_BAND_SCALE` (default `4.0`, multiplier on the derived band — bigger = visible longer), `GATE_CROP_OVERLAY_UPPER_BODY` (default `false`, crop only the top 50% / head+torso), `GATE_CROP_OVERLAY_ALLOW_UPSCALE` (default `false`; when false the PiP is clamped to the native crop size so it only ever downscales the source). The connector geometry uses the full bbox even when `UPPER_BODY` crops only the top half.
...
**Leftover-frame replies for clip-bearing videos.** A `gate_crossing`/`significant_motion` video can also carry leftover low-motion frames in `event_frames` (from secondary events), which the early-return analysis path ignores. After the primary highlight is sent, `main.handle_event` fires `schedule_extra_frame_reply` (fire-and-forget `asyncio` task): it runs `analyze_frames_local` in the `llm` lane (so the highlight is never delayed), and if a description comes back, posts the best frame (`_pick_best_frame`) as a threaded reply to the highlight via `telegram_notification.send_extra_frame_reply` (photo + caption, **no button** — the parent highlight already has one). `send_notifications` returns the primary message so it can be the reply anchor. Sent only when the LLM returns a description (Ollama down → stays quiet). Gated by `SEND_GATE_EXTRA_FRAMES` (default `true`). Purpose: surface an occasionally-interesting frame (e.g. a poorly-tracked person of interest) that the gate-crossing path would otherwise drop.
...
**Random output format (`{format}` placeholder).** A prompt may contain a `{format}` placeholder (e.g. `config/prompt_frame_long_uk.txt`); `_apply_format` in `analyze_frame.py` substitutes it **per call** with one of two format snippets, chosen by a real coin flip in code — with probability `OLLAMA_HOKKU_PROBABILITY` (0–1, default `0` = always prose) it picks the hokku snippet, else prose. The randomness MUST be in code: an LLM can't make a fair random choice (greedy/low-temp decoding collapses "pick a number 1-20" to a constant — almost always 13), so asking the model to roll a die never varies. Snippets: `config/prompt_frame_fmt_prose_uk.txt` (override `OLLAMA_FORMAT_PROSE_FILE`) and `config/prompt_frame_fmt_hokku_uk.txt` (override `OLLAMA_FORMAT_HOKKU_FILE`). The shared scene/subject/anti-russism rules stay in the prompt body; only the format line is swapped. Applied at every single/multi prompt load and is a no-op for prompts without `{format}` (short/refine/Gemini). Each round logs the chosen format. `OLLAMA_HOKKU_PROBABILITY=1` forces hokku (handy for testing).
